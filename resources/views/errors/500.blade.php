...
<head>
    ...
    <title>{{ __('Server Error') }} - {{ config('app.name', 'DancyMeals') }}</title>
...
<body class="bg-surface dark:bg-surface text-on-surface dark:text-on-surface font-sans antialiased">
    <div class="min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full text-center">
            <div class="mb-8">
                {{-- Warning icon in a danger-subtle circle --}}
                ...
                <h1 class="text-6xl font-bold text-on-surface-strong dark:text-on-surface-strong mb-2 font-display">500</h1>
                <h2 class="text-xl font-semibold text-on-surface-strong dark:text-on-surface-strong mb-4">{{ __('Server Error') }}</h2>
                <p class="text-on-surface dark:text-on-surface text-base leading-relaxed mb-6">
                    {{ __('Something went wrong on our end. Please try again in a few moments.') }}
                </p>
            </div>
