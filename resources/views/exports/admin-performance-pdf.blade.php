...
<title>{{ __('Cook Performance Export') }}</title>
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', Arial, sans-serif; font-size: 12px; color: #1a1a2e; background: #fff; padding: 24px; }
    .header { display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid #8b5cf6; padding-bottom: 16px; margin-bottom: 24px; }
    ...
    thead tr { background: #8b5cf6; color: #fff; }
...
<div class="title">{{ __('Cook Performance Metrics Report') }}</div>
<div class="subtitle">
        ...
        <tbody>
            @foreach($rows as $row)
                <tr>
                    ...
                    <td class="text-right">{{ number_format($row['net_revenue']) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<div class="footer">
    {{ __('DancyMeals — Cook Performance Export') }} &middot; {{ $generatedAt->format('Y') }}
</div>
