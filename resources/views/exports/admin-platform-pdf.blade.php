...
<title>{{ $title ?? __('Platform Analytics Export') }}</title>
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .summary-card { flex: 1; min-width: 120px; background: #f0fdfa; border: 1px solid #99f6e4; border-radius: 8px; padding: 12px 16px; }
...
<div class="header">
    ...
    <div class="meta">
        ...
        <div><strong>{{ __('Role') }}:</strong> {{ __('Platform Admin') }}</div>
    </div>
</div>

<div class="title">{{ $title ?? __('Platform Analytics Report') }}</div>
<div class="subtitle">
...
@if($hasData)
    <div class="summary">
        <div class="summary-card">
            <div class="label">{{ __('Gross Revenue') }}</div>
            <div class="value">{{ \App\Services\AnalyticsExportService::formatXAF($totalGross) }}</div>
        </div>
        <div class="summary-card">
            <div class="label">{{ __('Commission') }}</div>
            <div class="value">{{ \App\Services\AnalyticsExportService::formatXAF($totalCommission) }}</div>
        </div>
        <div class="summary-card">
            <div class="label">{{ __('Net Payout') }}</div>
            <div class="value">{{ \App\Services\AnalyticsExportService::formatXAF($totalNet) }}</div>
        </div>
        <div class="summary-card">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="text-right">{{ __('Commission (XAF)') }}</th>
                <th class="text-right">{{ __('Net Payout (XAF)') }}</th>
...
<div class="footer">
    {{ __('DancyMeals — Platform Analytics Export') }} &middot; {{ $generatedAt->format('Y') }}
</div>
