{{--
    ...
    Allows the cook to create reusable schedule templates with pre-configured
    time intervals for order, delivery, and pickup. Templates can be applied
    to multiple days at once (via F-105).
    ...
    BR-133: Permission-based access (can-manage-schedules)
    BR-134: Creation logged via Spatie Activitylog
...
>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold text-on-surface-strong">{{ __('Create Template') }}</h2>
            <p class="text-sm text-on-surface/60 mt-1">
                {{ __('Create reusable schedule templates to quickly configure multiple days.') }}
            </p>
        </div>
        <a
    ...
    <div class="bg-surface-alt dark:bg-surface-alt rounded-lg border border-outline dark:border-outline shadow-card overflow-hidden">
        <form @submit.prevent="$action('{{ url('/dashboard/schedule/templates') }}')">
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-outline dark:border-outline bg-surface/50 dark:bg-surface/30">
                ...
                <button
                    ...
                    :disabled="$fetching()"
                >
                    <span x-show="!$fetching()">{{ __('Create Template') }}</span>
    ...
    @if($templateCount > 0)
        <div class="mt-8">
            <h3 class="text-base font-semibold text-on-surface-strong mb-4 flex items-center gap-2">
                <svg class="w-5 h-5 text-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>
                {{ __('Existing Templates') }}
                <span class="text-sm font-normal text-on-surface/60">({{ $templateCount }})</span>
            </h3>
                ...
                @foreach($templates as $template)
                    <div class="bg-surface-alt dark:bg-surface-alt rounded-lg border border-outline dark:border-outline p-4 shadow-card">
                        <div class="flex items-start justify-between mb-2">
                            <h4 class="text-sm font-semibold text-on-surface-strong">{{ $template->name }}</h4>
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center gap-2 text-xs text-on-surface/60">
                                <svg class="w-3.5 h-3.5 text-info shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                                <span>{{ __('Orders') }}: {{ $template->order_interval_summary }}</span>
                            </div>
                            @if($template->delivery_interval_summary)
                                <div class="flex items-center gap-2 text-xs text-on-surface/60">
                                    <svg class="w-3.5 h-3.5 text-primary shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"></path><path d="M15 18H9"></path><path d="M19 18h2a1 1 0 0 0 1-1v-3.65a1 1 0 0 0-.22-.624l-3.48-4.35A1 1 0 0 0 17.52 8H14"></path><circle cx="7" cy="18" r="2"></circle><circle cx="17" cy="18" r="2"></circle></svg>
                                    <span>{{ __('Delivery') }}: {{ $template->delivery_interval_summary }}</span>
                                </div>
                            @endif
                            @if($template->pickup_interval_summary)
                                <div class="flex items-center gap-2 text-xs text-on-surface/60">
                                    <svg class="w-3.5 h-3.5 text-secondary shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9h18v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9Z"></path><path d="m3 9 2.45-4.9A2 2 0 0 1 7.24 3h9.52a2 2 0 0 1 1.8 1.1L21 9"></path><path d="M12 3v6"></path></svg>
                                    <span>{{ __('Pickup') }}: {{ $template->pickup_interval_summary }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
