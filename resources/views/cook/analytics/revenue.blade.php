...
<div
    x-data="{
        ...
        buildExportUrl(format) {
            let url = '/dashboard/analytics/export-' + format + '?period=' + this.period;
            if (this.period === 'custom' && this.customStart && this.customEnd) {
                url += '&custom_start=' + this.customStart + '&custom_end=' + this.customEnd;
            }
            return url;
...
>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        {{-- Export Button (F-208) — top-right per UI/UX spec --}}
        {{-- x-navigate-skip prevents Gale interception (F-058 pattern) --}}
        <div class="relative" x-data @click.outside="showExportMenu = false">
            <button
                type="button"
            ...
            <div
                ...
                x-cloak
                class="absolute right-0 mt-1 w-48 bg-surface border border-outline rounded-xl shadow-dropdown z-20 py-1 dark:bg-surface dark:border-outline"
                role="menu"
            >
                {{-- Export as CSV --}}
                <a
                    ...
                    x-navigate-skip
                    class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-on-surface hover:bg-surface-alt dark:hover:bg-surface-alt transition-colors duration-100"
                    role="menuitem"
                    @click="showExportMenu = false"
                >
                    <svg class="w-4 h-4 text-success shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <a
                    ...
                    @click="showExportMenu = false"
                >
                    <svg class="w-4 h-4 text-danger shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
