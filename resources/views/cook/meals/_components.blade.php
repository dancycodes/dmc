{{--
    ...
    Displays existing components and provides a form to add new ones.
...
<div
    ...
    x-sync="['comp_name_en', 'comp_name_fr', 'comp_price', 'comp_selling_unit', 'comp_min_quantity', 'comp_max_quantity', 'comp_available_quantity']"
>
    {{-- Section header --}}
    ...
    @if(($componentData['count'] ?? 0) > 0)
        <div class="space-y-3">
            @foreach($componentData['components'] as $component)
                <div class="flex items-center justify-between p-3 rounded-lg border border-outline dark:border-outline bg-surface dark:bg-surface hover:bg-surface-alt dark:hover:bg-surface-alt transition-colors duration-200">
                    ...
                    {{-- Action buttons (stubs for F-119, F-120, F-123) --}}
                    <div class="flex items-center gap-1 ml-3 shrink-0">
                        {{-- Edit (F-119) --}}
                        <span
                            class="p-1.5 rounded text-on-surface/30 cursor-not-allowed"
                            title="{{ __('Edit coming soon') }}"
                        >
