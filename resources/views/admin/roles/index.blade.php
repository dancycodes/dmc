    ...
    >
        ...
        @if($roles->count() > 0)
            ...
            <div class="md:hidden space-y-3" x-data x-navigate>
                @foreach($roles as $role)
                    <a href="{{ url('/vault-entry/roles/' . $role->id . '/edit') }}" class="block bg-surface-alt dark:bg-surface-alt rounded-lg border border-outline dark:border-outline p-4 hover:border-primary/30 transition-colors">
