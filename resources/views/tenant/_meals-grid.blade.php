{{--
    ...
    F-136: Meal Filters — Fragment wrapper for filter updates
    ...
    Edge case: No meals match combined filters shows descriptive message
--}}
...
<div id="meals-grid-fragment">
    ...
    @if($meals->total() > 0)
        ...
        <div class="mt-4 text-center">
            <p class="text-sm text-on-surface/60">
                @if(!empty($currentSearch) && $hasActiveFilters)
                    ...
                    {{ __('with filters applied') }}
                @elseif(!empty($currentSearch))
                    {{ trans_choice(':count result|:count results', $meals->total(), ['count' => $meals->total()]) }}
                    {{ __('for') }} "<span class="font-medium text-on-surface-strong">{{ $currentSearch }}</span>"
    ...
    @else
        ...
        <div class="text-center py-12">
            ...
            @if(!empty($currentSearch) && $hasActiveFilters)
                {{-- Combined search + filters empty state --}}
                <p class="text-on-surface/60 text-lg font-medium">
                    {{ __("No meals found for ':query' with your filters.", ['query' => $currentSearch]) }}
                </p>
                <p class="text-on-surface/40 text-sm mt-1">
                    {{ __('Try adjusting your filters or search with a different keyword.') }}
                </p>
            @elseif(!empty($currentSearch))
            ...
            @elseif($hasActiveFilters)
                {{-- F-136 Edge case: No meals match filters --}}
                <p class="text-on-surface/60 text-lg font-medium">
                    {{ __('No meals match your selected filters.') }}
                </p>
                <p class="text-on-surface/40 text-sm mt-1">
