{{--
    Email Verification Template (N-021, BR-038)

    Extends the base DancyMeals email layout for consistent branding.
    Contains greeting, verification button, expiration notice, and footer text.
...
@section('preheader')
{{ __('Verify your email address to get started with DancyMeals.', [], $emailLocale ?? 'en') }}
@endsection
...
@section('content')
    ...
    <p class="email-content-text" style="margin: 0 0 24px 0; font-size: 15px; color: #3F3F46; line-height: 1.6;">
        {{ __('Thank you for creating your DancyMeals account! Please verify your email address by clicking the button below.', [], $emailLocale ?? 'en') }}
    </p>
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin: 0 auto 24px auto;">
        <tr>
            <td style="border-radius: 8px; background-color: #0D9488;">
                <a href="{{ $verificationUrl }}" class="btn-primary" target="_blank" style="display: inline-block; padding: 14px 32px; font-size: 16px; font-weight: 600; color: #FFFFFF; text-decoration: none; border-radius: 8px; background-color: #0D9488;">
                    {{ __('Verify Email Address', [], $emailLocale ?? 'en') }}
                </a>
            </td>
        </tr>
    </table>
    ...
    <p class="email-content-text" style="margin: 0 0 16px 0; font-size: 14px; color: #71717A; line-height: 1.5;">
        {{ __('This verification link will expire in :minutes minutes.', ['minutes' => $expirationMinutes ?? 60], $emailLocale ?? 'en') }}
    </p>
